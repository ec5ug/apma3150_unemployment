...
decision_tree <- rpart(Approving ~ UNRATE + CPIAUCSL + M2REAL + PCE, 
                       data = train_data, 
                       method = "anova")

pruned_tree <- prune(decision_tree, cp = 0.025)
...
rmse <- sqrt(mean((predictions - test_data$Approving)^2, na.rm = TRUE))
mae <- mean(abs(predictions - test_data$Approving), na.rm = TRUE)
